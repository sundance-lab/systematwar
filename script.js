const CONFIG = {
    ...
    BUILDINGS: {
        ...
        PlanetaryDefenses: {
            ...
            unitBonus: 0,
            incomeBonus: 0,
            defenseBonus: 0.2 // 20% stronger defense
        }
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    closeControlPanelXButton = document.getElementById('close-control-panel-x'); // Renamed
    ...
    modalInputConfirm.addEventListener('click', () => {
        ...
        if (modalCallback) {
            ...
            if (!isNaN(parsedValue) && value.trim() !== '') { // If it's a valid number input
                modalCallback(parsedValue); 
            } else { // It's a string input for building type
                modalCallback(value);
            }
            modalCallback = null;
        }
    ...
    playButton.addEventListener('click', () => {
        ...
        playerIncome = 500; // Start income at 500
        activeFleets = [];
            ...
            if (selectedSourcePlanet) { // Phase 2: Source already selected, looking for target
                    ...
                    if (clickedPlanet === selectedSourcePlanet) {
                        ...
                        if (selectedSourcePlanet.units === 0) {
                            ...
                            showModal(`Send units from ${sourceForCallback.name} (${sourceForCallback.units} units) to invade ${targetForCallback.name} (currently ${targetForCallback.owner} with ${targetForCallback.units} units)?\n\nEnter number of units:`, 'prompt', (unitsToSend) => {
                                // unitsToSend is now a string (from text input), need to parse
                                ...
                                if (isNaN(parsedUnitsToSend) || parsedUnitsToSend <= 0) {
                                    ...
                                    // Make sure to reset tempFixedInvasionLine and selectedSourcePlanet if input is invalid
                ...
                if (clickedPlanet) {
                    ...
                    if (!camera.targetPlanet) {
                        ...
                        lastMouseX = mouseX;
                        lastMouseY = mouseY;
                        canvas.style.cursor = 'default'; // Changed to default
                    }
                }
            }
        }
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        if (!isDragging && !isDrawingInvasionLine && canvas.style.cursor !== 'default') {
            ...
            canvas.style.cursor = 'crosshair';
        } else if (isDragging && canvas.style.cursor !== 'default') { // Changed to default
            canvas.style.cursor = 'default';
        }
    });
