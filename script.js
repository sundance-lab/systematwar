...
function animateSolarSystem() {
    ...
    for (let i = activeFleets.length - 1; i >= 0; i--) {
        ...
        if (fleet.progress >= 1) {
            handleFleetArrival(fleet); // NEW: Use generic handler
    ...
    currentPlanets.forEach(planet => {
        ...
        const nameOffset = planet.radius + Math.max(10, 15 / camera.zoom);
        ...
        ctx.font = `${unitsFontSize}px Arial`;
        ctx.fillStyle = '#0a0'; // Darker green for units
        const unitsOffset = nameOffset + Math.max(8, 10 / camera.zoom);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (!gameActive || gameModalBackdrop.classList.contains('active') || e.target !== canvas) {
            return;
        }
            ...
            if (selectedSourcePlanet) { // Phase 2: Source already selected, looking for target
                if (clickedPlanet) {
                    if (clickedPlanet === selectedSourcePlanet) {
                        showModal("Cannot send units to the same planet!", 'alert'); // Clarified message
                ...
                if (clickedPlanet) {
                    if (clickedPlanet.owner === 'player') {
                        ...
                        showModal("You can only initiate operations from your own planets!", 'alert'); // Generic message for non-player planets
                    }
