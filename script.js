const CONFIG = {
    ...
    PLANET_COUNTER_UPDATE_INTERVAL_MS: 1000, // Renamed from original, now used for general production interval
    ...
    PLANET_BASE_UNIT_PRODUCTION_RATE: 5, // Base units generated by a planet per interval
    PLANET_BASE_INCOME_PRODUCTION_RATE: 1, // Base income generated by a planet per interval
    ...
    // Removed INCOME_GENERATION_PER_PLAYER_PLANET
    ...
    // NEW: Building Costs and Effects
    BUILDINGS: {
        Garrison: {
            cost: 200,
        ...
        // Market District and Planetary Defenses can be added here later
    }
};
...
function initSolarSystem() {
    ...
    for (let i = 0; i < numPlanets; i++) {
        ...
        currentPlanets.push({
            ...
            owner: ownerType,
            units: initialUnits,
            // NEW: Production rates and buildings
...
function generatePlanetProduction() {
    currentPlanets.forEach(planet => {
        // Apply building effects to production rates (for future Market/Defense)
        ...
        planet.buildings.forEach(buildingName => {
            ...
            // Add checks for Market District and Planetary Defenses effects here later
        });
...
function showPlanetControlPanel(planet) {
    ...
    panelUnitProductionRateDisplay.textContent = planet.unitProductionRate;
    panelIncomeRateDisplay.textContent = planet.incomeProductionRate;
    ...
    for (let i = 0; i < numSlots; i++) {
        ...
        if (buildingInSlot) {
            ...
            // For now, only allow building Garrison
            const buildButton = document.createElement('button');
            buildButton.textContent = `Build Garrison (${CONFIG.BUILDINGS.Garrison.cost} income)`;
            buildButton.addEventListener('click', () => {
                if (playerIncome >= CONFIG.BUILDINGS.Garrison.cost) {
                    playerIncome -= CONFIG.BUILDINGS.Garrison.cost;
                    planet.buildings[i] = 'Garrison'; // Store building name
                    planet.unitProductionRate += CONFIG.BUILDINGS.Garrison.unitBonus; // Apply immediate effect
                    updatePlayerIncomeDisplay(); // Update global income display
                    showModal(`Garrison built on ${planet.name}!`, 'alert');
                    showPlanetControlPanel(planet); // Re-render panel to show new building
                } else {
                    showModal("Not enough income to build Garrison!", 'alert');
                }
            });
            slotDiv.appendChild(buildButton);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    planetControlPanel.addEventListener('mousedown', (e) => {
        if (e.button === 0) { // Left mouse button
            ...
            planetControlPanel.style.cursor = 'pointer'; // Changed to pointer
    ...
    playButton.addEventListener('click', () => {
        ...
        camera.targetZoom = CONFIG.CAMERA_INITIAL_ZOOM; // Reset targetZoom to initial zoom level
        playerIncome = 0;
        activeFleets = [];
        ...
        camera.targetZoom = CONFIG.CAMERA_INITIAL_ZOOM; // Ensure target zoom is initial
        ...
        // Renamed planetCounterInterval to planetProductionInterval
        if (planetProductionInterval) clearInterval(planetProductionInterval);
        planetProductionInterval = setInterval(generatePlanetProduction, CONFIG.PLANET_COUNTER_UPDATE_INTERVAL_MS);
    });
            ...
            if (selectedSourcePlanet) { // Phase 2: Source already selected, looking for target
                ...
                if (clickedPlanet) {
                    ...
                    if (!camera.targetPlanet) {
                        ...
                        lastMouseX = mouseX;
                        lastMouseY = mouseY;
                        canvas.style.cursor = 'pointer'; // Changed to pointer
                    }
                }
            }
        }
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        if (!isDragging && !isDrawingInvasionLine && canvas.style.cursor !== 'default') {
            ...
            canvas.style.cursor = 'crosshair';
        } else if (isDragging && canvas.style.cursor !== 'pointer') { // NEW: cursor for dragging
    ...
    canvas.addEventListener('mouseup', (e) => {
        ...
        planetControlPanel.style.cursor = 'pointer'; // Reset panel cursor
        canvas.style.cursor = 'default'; // Reset canvas cursor after drag
    });
    ...
    canvas.addEventListener('mouseleave', () => {
        ...
        isDraggingPanel = false; // Reset panel dragging
        planetControlPanel.style.cursor = 'pointer'; // Reset panel cursor
        canvas.style.cursor = 'default'; // Reset canvas cursor after leaving
